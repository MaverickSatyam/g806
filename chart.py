# chart.py
# Email: 23f3003273@ds.study.iitm.ac.in
import seaborn as sns
import matplotlib.pyplot as plt
import pandas as pd
import numpy as np

# --------------------------
# Generate synthetic monthly revenue data
# --------------------------
months = pd.date_range(start="2023-01-01", periods=12, freq="M")
np.random.seed(0)
revenue = 100000 + np.random.normal(0, 10000, 12) + 10000 * np.sin(np.linspace(0, 2*np.pi, 12))

df = pd.DataFrame({"Month": months, "Revenue": revenue})

# --------------------------
# Seaborn styling
# --------------------------
sns.set_style("whitegrid")

# --------------------------
# Create figure exactly 512x512 pixels
# --------------------------
fig, ax = plt.subplots(figsize=(512/100, 512/100), dpi=100)  # 5.12in x 5.12in at 100 dpi = 512px
sns.lineplot(data=df, x="Month", y="Revenue", marker="o", ax=ax)

# Titles and labels
ax.set_title("Monthly Revenue", fontsize=12)
ax.set_xlabel("Month", fontsize=10)
ax.set_ylabel("Revenue (USD)", fontsize=10)

# Adjust margins to prevent Matplotlib from resizing
plt.subplots_adjust(left=0.15, right=0.95, top=0.85, bottom=0.15)

# Save PNG exactly 512x512 pixels
fig.savefig("chart.png", dpi=100, bbox_inches=None)
plt.close(fig)
